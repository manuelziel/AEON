...
void setup() {
  ...
  setupDefaults();                                         // set the defaults if EEPRROM have no values
...
*/
void setupDefaults() {
  if ((!rom.getInit()) && (rom.getErrorState() != EReturn_ROM::ERROR_EEPROM_COMMIT_FAILD)) {
    rom.setDefault(timer.getYear(), timer.getMonth(), timer.getDay(), 0 /*sex*/, rom.getDefaultLifespanWoman(), rom.getDefaultLifespanMan());
...
void loopPages() {
  ...
  switch (fsm.getCurrentStateId()) {
    case (EStates::STATE_Base):
      aeon.pageBase(now.year(), now.month(), now.day(), now.dayOfTheWeek(), now.hour(), now.minute(), now.second(), calcLifetime());
      break;

    case (EStates::STATE_Setup_Date_Set_Year):
      aeon.pageSetupDate_set_date(aeon.Year, now.year(), now.month(), now.day());
      break;

    case (EStates::STATE_Setup_Date_Set_Month):
      aeon.pageSetupDate_set_date(aeon.Month, now.year(), now.month(), now.day());
      break;

    case (EStates::STATE_Setup_Date_Set_Day):
      aeon.pageSetupDate_set_date(aeon.Day, now.year(), now.month(), now.day());
      break;
...
*/
int calcLifetime() {
  int lifespan = rom.getLifespan();  // ~79 - 86
